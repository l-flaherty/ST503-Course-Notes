...
$\mathbb{V}(c^T\hat{\beta})=c^T \text{Cov}(\hat{\beta})c$ when $X$ has full column rank. First 15 pages or so of Week 3 A, including interactive model (Estimation of linear functions of beta and Gauss-Markov Theorem in week 3 A around 15minutes in with R).

Among all the linera estimators of $c^T\beta$, a natural question is which is hte best (where best mean minimum variance)? **Gauss-Markov Theorem** says that under the Guass-Markov model, for an estimable $c^T\beta$, $c^T\hat{\beta}$ is the best linear unbiased estimator (BLUE).

### Goodness of fit

Recall that $\hat{y}=P_Xy$ and $\hat{\epsilon}=(I-P_X)y$, so $y=\hat{y}+\hat{\epsilon}$. We can write this for each term as $y_i=\hat{y_i+(y_i-\hat{y_i})}$, and then $y_i-\overline{y}=(\hat{y_i}-\overline{y})+(y_i-\hat{y_i})$ and so $\sum(y_i-\overline{y})^2=\sum(\hat{y_i}-\overline{y})^2+\sum(y_i-\hat{y_i})^2$. The $\sum(y_i-\overline{y})^2$ term is called the **total sum of squares (TSS)**. The $\sum(\hat{y_i}-\overline{y})^2$ term is called the **model sum of squares (MSS)** (or sometimes model sum of squares). In an ideal world, you want your **residual sum of squares (RSS)**, the $y_i-\hat{y_i}$ terms, to be close to zero. Thus, we want the regression sum of squares to be close to the total sum of squares. The ratio of these two is $r^2=\frac{MSS}{TSS}=\frac{\sum(\hat{y_i}-\overline{y})^2}{\sum(y_i-\overline{y})^2}$. Since the terms in the above quality are all squared, the regression sum of squares can never be more than the total sum of squares, $r^2 \leq 1$. Similar, since the regression sum of squares can never be negative, $r^2\geq 0$ (if so, there is no fit at all, everything comes from the error terms). We can then interpret $r^2$ to be the proportion of variation captured by the model. Another interpretation of $r^2$ is literally the square of the correlation between the observed and predicted values of $y$; $r^2=\text{Corr}(y, \hat{y})^2=\left(\frac{\text{Cov}(y, \hat{y})}{\sqrt{\mathbb{V}(y)\mathbb{V}(\hat{y})}}\right)^2=\left(\frac{\mathbb{E}(y\hat{y})-\mathbb{E}(y)\mathbb{E}(\hat{y})}{\sqrt{\mathbb{V}(y)\mathbb{V}(\hat{y})}}\right)^2$.

Let's go back to our previous example of predicting gambling habits. We want to verify our calculation for $r^2$, $\frac{\sum(\hat{y_i}-\overline{y})^2}{\sum(y_i-\overline{y})^2}$ matches the output to our model. Note that while `summary(gamble_regression)$r.squared` directly gives us the computed $r^2$, we can also read it directly off the second to last line in `summary(gamble_regression)`

```{r}
library(faraway)
gamble_regression=lm(gamble~., data=teengamb)

yhat=predict(gamble_regression)
yobs=teengamb$gamble
yavg=mean(teengamb$gamble)

MSS=sum((yhat-yavg)^2)
TSS=sum((yobs-yavg)^2)

compare_r2=data.frame(direct=MSS/TSS, lm=summary(gamble_regression)$r.squared)
compare_r2

```

### Addional Assumption to Gauss-Markov

Thus far we have assumed our model $y=X\beta+\epsilon$ is such that $\mathbb{E}(\epsilon)=0$ and $\mathbb{V}(\epsilon)=\sigma^2I$. From now, on, we'd like to make an additional assumption on the *distribution* of the errors, namely that $\epsilon \in \mathbb{R}^n$ follows a multivariate normal distribution with mean $0 \in \mathbb{R}^n$ and Covariance matrix $\sigma^2I$ (i.e. $\text{Cov}(\epsilon_i, \epsilon_j)=0$ when $i \neq j$). In such a model, by the rules of expectation and variance, $y \sim N(X\beta, \sigma^2I)$. This means that each individual $y_i$ is normal with mean $X\beta_i$ and variance $\sigma^2$. Using previous results for our least-squares solution in the Gauss-Markov model, if $X$ has full column rank, then under this additional assumption, $\hat{\beta} \sim N(\beta, \sigma^2(X^TX)^{-1})$.

XXXXXX WHEN get more time, show with "Week 3- Testing in R" that this is indeed the case (centered at true, variance whatever)XXXXXXXXXXX

### Hypothesis testing for single coefficient (t-test)

Our model is $y_i=\beta_0+X_{.,i}\beta_1+\epsilon_i$ where $\epsilon_i \stackrel{iid}{\sim}N(0, \sigma^2)$. We want to test the hypothesis $H_0: \beta_1=0$ against the alternative $H_a: \beta_1 \neq 0$. We know that $\hat{\beta_1} \sim N(\beta_1, \sigma^2(X^TX)^{-1}_{2_2})$. If we standardize $\hat{\beta_1}$ by subtracting the mean and dividing by the standard deviation, we know that $\frac{\hat{\beta_1}-\beta_1}{\text{SE}(\hat{\beta_1})}=\frac{\hat{\beta_1}-\beta_1}{\sqrt{\sigma^2(X^TX)^{-1}_{2_2}}} \sim N(0,1)$. Unfortunately, this is not easily used since $\sigma^2$ is unknown. Instead, we must estimate it with $\hat{\sigma^2}$.

Our statistic of interest then becomes $\frac{\hat{\beta_1}-\beta_1}{\sqrt{\hat{\sigma^2}(X^TX)^{-1}_{2_2}}}$, and we want to see how this behaves under the assumption that $\beta_1=0$. This is called our test-statistic, $t=\frac{\hat{\beta_1}}{\hat{\text{SE}}(\hat{\beta_1})}$. If our null hypothesis that $\beta_1=0$ is really true, then $t$ follows a t-distribution with $n-\text{rank}(X)$ degrees of freedom; $t \sim t_{n-\text{rank}(X)}$. As would be expected, we reject our null hypothesis in favor of the alternative if $|t|$ is greater than our critical value (where $\alpha$ is our significance level, we reject $H_0$ when $|t| > t_{\frac{\alpha}{2}, n-\text{rank}(X)}$). Equivalently, we reject when our p-value (the probability of observing data as or more extreme under the null hypothesis) is less than our significance level.

XXXX when get time look at R-code for t-testXXXx

### Hypothesis testing for multiple coefficients (F-test)

Suppose we have multiple covariates (for simplicity, we look at the case of two predictors, but the following extends to higher dimensions as well). Our model is $yi=X\begin{bmatrix}\beta_0\\\beta_1 \\ \beta_2\end{bmatrix}+\epsilon$. We now want to test the hypothesis that $H_0: \beta_1=\beta_2=0$. Our reduced model under $H_0$ is $y_i=\beta_0+X_{i,1}\beta_1+\epsilon_i$. Our hypothesis is based on comparing the full model and reduced model.

Recall that, in general, we have $\left[\sum(y_i-\overline{y})^2=\sum(\hat{y_i}-\overline{y})^2+\sum(y_i-\hat{y_i})^2\right]=\left[SST=SSM+RSS\right]$. If the null hypothesis is true, that is if $\beta_2=\beta_3=0$, then the difference between the residual sum of squares in the reduced model and the residual sum of squared in the full model should be small.

Our test statistic is now $F=\frac{\frac{RSS_{\text{red}}-RSS_{\text{full}}}{\text{rank}(X_{\text{full}})-\text{rank}(X_{\text{red}})}}{\frac{RSS_{\text{full}}}{n-\text{rank}(X_{\text{full}})}}$. Under the null hypothesis, $F \sim F_{d_1, d_2}$ where $d_1=\text{rank}(X_{\text{full}})-\text{rank}(X_{\text{red}})$ and $d_2=n-\text{rank}(X_{\text{full}})$ and we proceed to reject/fail to reject as normal.

XXXX when get time look at R-code for F-testXXXXX

#Week 4

### Hypothesis Testing For Single Linear Function

We have already explored hypothesis testing for regression coefficients. Now we turn our attention to hypothesis testing for linear functions. We want a test the hypothesis $H_0: c^T\beta=0$ against the alternative $H_a: c^T\beta \neq 0$. Throughout this section we assume our model is Gauss-Markov with normality, and that $c^t\beta$ is actually estimable.

We have previously shown that $\hat{\beta} \sim N(\beta, V)$ for some covariance matrix $V$, and that the BLUE of $c^T\beta$ is $c^T\hat{\beta}$ which follows a normal distribution with mean $c^T\beta$ and covariance matrix $c^TVc$. As with before, the problem is that $V=\sigma^2(X^TX)^{-1}$ depends on the unknown $\sigma^2$, and so again as before, we estimate $V$ with $\hat{V}=\hat{\sigma^2}(X^TX)^{-1}$. Scaling and recentering to normalize, we get a similar test statistic as before, $\frac{c^T\hat{\beta}-c^T\beta}{\sqrt{c^T\hat{V}c}}$. Under the null hypothesis, our statistic is $t=\frac{c^T\hat{\beta}}{\sqrt{c^T\hat{V}c}}$ which follows a $t$-distribution with $n-\text{rank}(X)$ degrees of freedom.

### Hypothesis Testing For Multiple Linear Functions

One example is the one-way ANOVA. Our model is $y_{i,j}=\mu+\alpha_i+\epsilon_{i,j}$ where $\epsilon_{i,j} \stackrel{iid}{\sim}N(0, \sigma^2)$ for $i=1,2,3$, $j=1,\dots,5$. Our test is if all the groups have the same mean-- $\alpha_1=\alpha_2=\alpha_3$. One way to write our null hypothesis is $\begin{bmatrix} \alpha_1-\alpha_2 \\ \alpha_1-\alpha_3\end{bmatrix}=\begin{bmatrix} 0 \\ 0\end{bmatrix}$. Since our regression coefficients are $\beta=\begin{bmatrix} \mu & \alpha_1 & \alpha_2 & \alpha_3\end{bmatrix}^T$, our hypothesis is $H_0: K\beta=\begin{bmatrix} 0 & 1 & -1 & 0\\ 0 & 1 & 0 & -1 \end{bmatrix} \begin{bmatrix} \mu \\ \alpha_1 \\ \alpha_2 \\ \alpha_3 \end{bmatrix}=0$ against the alternative $H_a: K\beta \neq 0$. This is the **general linear hypothesis** and the assumption is that each linear function in $K\beta$ is estimable.

As expected, an estimator of $K\beta$ is $K\hat{\beta}$, which has a multinomial distribution with mean $K\beta$ and variance $KVK^T$ where $V=\sigma^2(X^TX)^-$, which is estimated by $\hat{V}=\hat{\sigma^2}(X^TX)^-$.

Our test statistic is as expected $F=\frac{(K\hat{\beta}-0)^T(K\hat{V}K^T)^{-1}(K\hat{\beta}-0)}{s}$ where $s$ denotes the number of rows in $K$. The test-statistic follows an $F$ distribution with $s, n-\text{rank}(X)$ degrees of freedom.

xxxxx again go through R when get more timexxxxx
left off at week 4 part 2

### Confidence Intervals

From earlier work, under the assumption that $c^T\beta$ is estimable, our BLUE is $c^T\hat{\beta}$., which follows a normal distribution with mean $c^T\beta$ and covariance matrix $c^T\sigma^2(X^TX)^-C$. Rescaling gives us a standard normal, $\frac{c^T\hat{\beta}-c^T\beta}{\sqrt{c^TvC}} \sim N(0,1)$ where $V=\sigma^2(X^TX)^-$. Since we don't actually know $\sigma^2$, we estimate it and have $\frac{c^T\hat{\beta}-c^T\beta}{\sqrt{c^T\hat{V}C}} \sim t_{n-\text{rank}(X)}$

If we want to be $95\%$ confident of where this statistic lies, we can look at the symmetry of of the $t$-distribution. We want $2.5\%$ on either side of statistic, so have $\mathbb{P}\left(-k \leq \frac{c^T\hat{\beta}-c^T\beta}{\sqrt{c^T\hat{V}C}} \leq k\right)=0.95$ where $K=t_{0.025, n-\text{rank}(X)}$. Solving for the linear functional, we have $\mathbb{P}\left(c^T\beta \in (c^T\hat{\beta} \pm k\sqrt{c^T\hat{V}c})\right)=0.95$. This is called a $95\%$ **confidence interval** of $c^T\beta$. As expected, we set our confidence level to $1-\alpha$ where $\alpha$ is our significance level/false positive rate/ Type I error rate. Of course, we can use confidence intervals along with hypothesis tests-- if we set our significance level at $5\%$, then it of observed value is in the $95\%$ confidence interval, we fail to reject.  

xxxR example in 

### Prediction Intervals

We have a model $y=X\beta+\epsilon$ where $\epsilon \sim N(0, \sigma^2I)$. Suppose now that we have a new unit. We have the covariates (the predictors $x_i$), but want to predict the response (the $y_i$). It is instinctive that $\mathbb{E}(y_{new} | x_{new})=x_{new}^T\hat{\beta}$, which is our prediction.

There are two sources of variability in our prediction. The first is from the estimate of $\beta$. The second is the randomness in the $y_{\text{new}}$ term, since it has it's own error. What is the distribution of our prediction less our response (the residual)? It is $x_{new}^T\hat{\beta}-y_{new} \sim N(0, X_{new}^TVX_{new}+\sigma^2)$ where $V=\sigma^2(X^TX)^-$. The variance comes from the regular variance rules: $\mathbb{V}(X_{new}^T\hat{\beta}-y_{new})=\mathbb{V}(X_{new}^T\beta)+\mathbb{V}(y_{new})-\text{Cov}(X_{new}^T\hat{\beta}, y_{\text{new}})=X_{\text{new}}^TVX_{\text{new}}+\sigma^2+0$. The covariance is zero since the new error is independent of the existing errors (this is our assumption). This means that the new $y_{\text{new}}$ is independent of the existing $y_i$'s.

We then can reshape and rescale as before-- $\frac{X_{\text{new}}^T\hat{\beta}-y_{\text{new}}}{\sqrt{X_{\text{new}}^TVX_{\text{new}}+\sigma^2}}\sim N(0,1)$ and $\frac{X_{\text{new}}^T\hat{\beta}-y_{\text{new}}}{\sqrt{X_{\text{new}}^T\hat{V}X_{\text{new}}+\sigma^2}}\sim t_{n-\text{rank}(X)}$. We then use this distribution to form our $100(1-\alpha)\%$ prediction interval. Our point estimate is $X_{\text{new}}^T\hat{\beta}$ and our margin of error is $\left(t_{\frac{\alpha}{2}, n-\text{rank}(X)}\right)\sqrt{X_{\text{new}}^T\hat{V}X_{\text{new}}+\hat{\sigma^2}}$. This is much wider than the confidence interval.

# Week 5

### Permutation test
Want to check normality assumption in the errors. Idea is to permute the response and keep the covariates, and compute the F stat each time. These computed values can be thought of as a random sample from the null distribution of the F-stat.

Our p-value is the proportion of the generated F-stats that exceed our F_obs, divided by the number of F-stats generated.

xxxR codexxx

### Diagnostics I
We have made four assumptions with our model $y=X\beta+\epsilon$. One, that the errors have constant variance. Two, that the errors are uncorrelated. Three, that the errors are normally distributed. Four, that there is a linear relationship between $x$ and $y$.

We don't actually know the error terms but have to use our residuals as a proxy, $\hat{\epsilon}=y-\hat{y}=(I-P_X)y$. Computing the covariance, we get $\text{Cov}(\hat{\epsilon})=\sigma^2=\sigma^2(I-P_X)$. This is a problem because even if our errors are independent, uncorrelated, and have the same variance,  the variance of the residuals aren't. 

We can plot the residuals on the y axis and fitted values on the x-axis to see if the spread changes based on the predicted fit. We we get a strong trend, then we would likely reject the constant variance assumption.

If that is the case, we move the **generalized least squares solution**. In such a case, we are aiming to minimize $(y-X\beta)^TW(y-X\beta)$ where $W$ is a diagonal matrix with weighted entries. Another option is to consider a transformation of $y$ such as $\log(y), \sqrt{y}, etc.$.
